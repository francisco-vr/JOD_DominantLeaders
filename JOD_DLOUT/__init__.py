...
class Player(BasePlayer):
    ...
    leader_message = models.StringField( 
        choices=[['No le destruyan dinero', 'No le destruyan dinero'],['Son libres de hacer lo que gusten','Son libres de hacer lo que gusten'],['Quizás es una buena idea destruirles más dinero', 'Quizás es una buena idea destruirles más dinero'],
    ...
    currentTimeLeader=models.StringField()
    currentTimeFollower=models.StringField()
...
class ShuffleWaitPage1(WaitPage):
    ...
    """ #Este trozo de código se comenta porque era sólo para testear sin player.Participant
    @staticmethod
...
class ShuffleWaitPage2(WaitPage):
    ...
    def after_all_players_arrive(subsession):
        ...
        for row in estructura:
            for player in row:
                if player.participant.group_assignment=="Grupo 1" and player.participant.group_role=="lider": #luego cambiar a player.Participant
        ...
        for row in estructura:
            for player in row:
                if player.participant.group_assignment=="Grupo 2" and player.participant.group_role == "lider": #luego cambiar a player.Participant
        ...
        for row in estructura:
            for player in row:
                if player.participant.group_assignment == "Grupo 1" and player.participant.group_role == "seguidor": #luego cambiar a player.Participant
                    group2_seguidores.append(player.participant.id_in_session)
                    

        for row in estructura:
            for player in row:
                if player.participant.group_assignment == "Grupo 2" and player.participant.group_role == "seguidor": #luego cambiar a player.Participant
...
class leader_message(Page):
    ...
    @staticmethod
    def is_displayed(player:Player):
        return player.participant.group_role == 'lider'  # después cambiar a player.Participant.group_role
    ...
    def before_next_page(player:Player, timeout_happened):
        ...
        for p in seguidores:
            if p.group_assignment == player.participant.group_assignment:
                print("p es: ", p)
...
class JOD_leader_results(Page):
    ...
    def before_next_page(player:Player, timeout_happened):
        ...
        for p in lista_jugadores:
            if p.group_role == 'lider': #puede que esto falle? será player.participant?
        ...
        for p in lista_jugadores: #aqui puede también que falle. p.participant?
