...
class ShuffleWaitPage(WaitPage):
    ...
    def after_all_players_arrive(self):
        ...
        for group_players in groups.values():
            sorted_players = sorted(group_players, key=lambda player: player.dom_scale_sum, reverse=True)

            top_players = sorted_players[:2]

            leader = max(top_players, key=lambda player: player.mean_punitiveness)
...
page_sequence = [minimal, SDP, punitiveness1, punitiveness2, punitiveness3, ShuffleWaitPage,Group1_follower, Group2_follower, Group1_leader, Group2_leader] 
